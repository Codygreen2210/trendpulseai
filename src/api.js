...
export function useAPI() {
  ...
  // ── Real Amazon trending bestsellers (Rainforest API) ─────────────────────
  const fetchTrending = useCallback(async (timeframe) => {
    ...
    if (!r.ok) {
      ...
      throw new Error(body.error || body.detail || `Trending failed (${r.status})`);
    }
    return r.json();
  }, [getToken]);

  // ── Amazon arbitrage deals (Rainforest API) — cheap, discounted flips ─────
  const fetchDeals = useCallback(async (timeframe) => {
    const token = await getToken();
    ...
    if (!r.ok) {
      const ct = r.headers.get("content-type") || "";
      const body = ct.includes("application/json") ? await r.json() : { error: await r.text() };
      throw new Error(body.error || body.detail || `Deals failed (${r.status})`);
    }
    return r.json();
  }, [getToken]);

  // ── Real-time Amazon search (Rainforest API) ──────────────────────────────
  const lookupAmazon = useCallback(async (query) => {
    const token = await getToken();
    const r = await fetch(`${BASE}/api/amazon-lookup`, {
      method: "POST",
      headers: {
        "Content-Type":  "application/json",
        "Authorization": `Bearer ${token}`,
    ...
    if (!r.ok) {
      const ct = r.headers.get("content-type") || "";
      const body = ct.includes("application/json") ? await r.json() : { error: await r.text() };
      throw new Error(body.error || body.detail || `Lookup failed (${r.status})`);
    }
    return r.json();
  }, [getToken]);
  ...
  return { callAI, fetchTrending, fetchDeals, lookupAmazon, startCheckout, openPortal };
}
