...
function TrendingProductsSection({ tier, accentColor, platforms, onAnalyze }) {
  ...
  const ranges = [
    { id: "24h", label: "24H",    description: "Today's top Amazon bestsellers" },
    { id: "7d",  label: "7 DAYS", description: "This week's top Amazon bestsellers" },
    { id: "30d", label: "30 DAYS",description: "This month's top Amazon bestsellers" },
  ];

  // Fetch real Amazon trending data from Rainforest API (cached 6 hours on backend)
  const fetchRange = useCallback(async (rangeId, force = false) => {
    ...
    try {
      const { products } = await fetchTrending(rangeId);
  ...
  }, [cache, fetchTrending, targetCount]);
  ...
  const progress     = ""; // no longer chunked — just single call
    ...
    <div style={{ marginTop:24 }}>
      ...
      <div style={{ marginBottom:14 }}>
        <div style={{ fontFamily:"Syne,sans-serif", fontWeight:900, fontSize:16, color:"#f0f0f0" }}>
          📊 Live <span style={{ color:accentColor }}>Amazon Bestsellers</span>
        </div>
        <div style={{ fontSize:10, color:"#444", marginTop:2 }}>
          Real-time data from Amazon · updated throughout the day
        </div>
      </div>
