...
function TrendingProductsSection({ tier, accentColor, platforms, onAnalyze, maxCount }) {
  const { callAI } = useAPI();
  const [activeRange, setActiveRange] = useState("7d");
  // Cache: { "24h": [...products], "7d": [...], "30d": [...] }
  ...
  const ranges = [
    { id: "24h", label: "24H",  description: "Yesterday's surge", promptText: "trending in the last 24 hours" },
    { id: "7d",  label: "7 DAYS", description: "This week's winners", promptText: "trending over the past 7 days" },
    { id: "30d", label: "30 DAYS", description: "This month's bestsellers", promptText: "best-selling over the past 30 days" },
  ];

  // Fetch products for a specific range, avoiding duplicates across ranges
  const fetchRange = useCallback(async (rangeId) => {
    if (cache[rangeId]?.length > 0) return; // already loaded
    setLoading(l => ({ ...l, [rangeId]: true }));
    setErrors(e => ({ ...e, [rangeId]: null }));

    // Build list of already-placed product names across other ranges
    const alreadyPlaced = Object.entries(cache)
      .filter(([r]) => r !== rangeId)
      .flatMap(([_, prods]) => (prods || []).map(p => p.name))
      .filter(Boolean);

    const range = ranges.find(r => r.id === rangeId);
    const avoidList = alreadyPlaced.length
      ? `\n\nDO NOT include these products (already placed in other tabs): ${alreadyPlaced.join(", ")}`
      : "";

    const prompt = `You are an Amazon product research analyst. List the top 20 most ${range.promptText} products on Amazon across all categories.${avoidList}

Return ONLY a raw JSON array (no markdown, no backticks) with 20 distinct products:
[
  {"id":1,"name":"Realistic product name","brand":"Brand","asin":"B0XXXXXXXXX","category":"Home","platforms":["Amazon"],"surge":85,"potential":88,"sales30d":45000,"reviews":12000,"rating":4.6,"price":"$29.99","origPrice":null,"onSale":false,"trend":"+120%","tags":["trending"],"description":"Brief one-line why it sells.","pros":["Strength 1","Strength 2","Strength 3"],"cons":["Risk 1","Risk 2"],"sourcing":{"wholesaleEst":"$8-$12","fbaFee":"$4.50","netMargin":"28-35%","sellerCount":45,"fulfillment":"FBA","moq":"50","leadTime":"3-5 weeks"}}
]

Use DIFFERENT products for each tab — a 24-hour trend should differ from a monthly bestseller. Vary categories: home, kitchen, beauty, tech, fitness, pets, office, outdoor, fashion. Realistic ASINs starting with B0.
Return ONLY the JSON array.`;
    ...
    try {
      ...
      const withIds = products.map((p, i) => ({
        ...p,
        id: `${rangeId}-${i}-${p.asin || p.name?.slice(0,10) || Math.random()}`,
      }));

      setCache(c => ({ ...c, [rangeId]: withIds }));
      setLastUpdated(u => ({ ...u, [rangeId]: Date.now() }));
    } catch (err) {
      console.error(`Failed to fetch ${rangeId} trends:`, err);
      setErrors(e => ({ ...e, [rangeId]: err.message }));
      // Fallback to seed data for this range
      const fallback = HOT_PRODUCTS
        .slice(rangeId === "24h" ? 0 : rangeId === "7d" ? 7 : 14, rangeId === "24h" ? 7 : rangeId === "7d" ? 14 : 21)
  ...
  // Load the currently active range on mount / when it changes
  useEffect(() => {
    if (!cache[activeRange]) fetchRange(activeRange);
  }, [activeRange, fetchRange, cache]);

  const currentProducts = (cache[activeRange] || []).slice(0, maxCount);
  const isLoading       = !!loading[activeRange];
  const hasFallback     = !!errors[activeRange] && cache[activeRange]?.length > 0;
  ...
  return (
    <div style={{ marginTop:24 }}>
      ...
      <div style={{ textAlign:"center", marginTop:18 }}>
        <button
          onClick={() => {
            setCache(c => ({ ...c, [activeRange]: undefined }));
          ...
          <div>
            ...
            {tier === "free" && (
              <div style={{ marginBottom:14, padding:"10px 14px",
                ...
                display:"flex", alignItems:"center", justifyContent:"space-between", gap:10 }}>
                <span>Free shows top 5.{" "}
                  <strong style={{ color:"#94a3b8" }}>Growth ($10/mo)</strong> unlocks top 20. All tiers can look up any Amazon product.
                </span>
            ...
            <TrendingProductsSection
              ...
              accentColor={ac}
              platforms={platforms}
              maxCount={tier==="free"?5 : tier==="growth"?20 : 999}
              onAnalyze={(prod, t) => { setAnalyzingProduct(prod); setAnalyzeType(t); }}
            />
          </div>
        )}
