...
function TrendingProductsSection({ tier, accentColor, platforms, onAnalyze }) {
  ...
  // cache[rangeId] = [...products], errors[rangeId] = "msg"
  const [cache, setCache]             = useState({});
  const [loading, setLoading]         = useState({});
  const [loadingProgress, setLoadingProgress] = useState({}); // e.g. "20/50"
  const [errors, setErrors]           = useState({});
  const [lastUpdated, setLastUpdated] = useState({});
  const [currentPage, setCurrentPage] = useState({ "24h": 1, "7d": 1, "30d": 1 });

  // Per-tier caps per tab. Free gets a taste; Pro gets effectively unlimited.
  const targetCount = tier === "free" ? 10 : tier === "growth" ? 50 : 100;
  const perPage = 10;

  const ranges = [
    { id: "24h", label: "24H",    description: "Yesterday's surge",         promptText: "trending in the last 24 hours (today's breakouts)" },
    { id: "7d",  label: "7 DAYS", description: "This week's winners",       promptText: "trending over the past 7 days (this week's momentum)" },
    { id: "30d", label: "30 DAYS",description: "This month's bestsellers",  promptText: "best-selling over the past 30 days (monthly heavy hitters)" },
  ];

  // Fetch a chunk of products. Returns parsed array or [].
  const fetchChunk = useCallback(async (range, avoidList, offset, chunkSize) => {
    const avoidStr = avoidList.length
      ? `\n\nDO NOT include any of these already-placed products: ${avoidList.slice(0,100).join(", ")}`
      : "";

    const prompt = `You are an Amazon product research analyst. List ${chunkSize} distinct products ${range.promptText}, sorted by sales potential (highest first).${avoidStr}

Skip the first ${offset} products in your internal ranking and return products ${offset+1} through ${offset+chunkSize}.

Return ONLY a raw JSON array (no markdown, no backticks). ${chunkSize} products:
[
  {"name":"Realistic product name","brand":"Brand","asin":"B0XXXXXXXXX","category":"Home","platforms":["Amazon"],"surge":85,"potential":88,"sales30d":45000,"reviews":12000,"rating":4.6,"price":"$29.99","origPrice":null,"onSale":false,"trend":"+120%","tags":["trending"],"description":"Brief one-line why it sells.","pros":["Strength 1","Strength 2","Strength 3"],"cons":["Risk 1","Risk 2"],"sourcing":{"wholesaleEst":"$8-$12","fbaFee":"$4.50","netMargin":"28-35%","sellerCount":45,"fulfillment":"FBA","moq":"50","leadTime":"3-5 weeks"}}
]

Mix categories widely: home, kitchen, beauty, tech, fitness, pets, office, outdoor, fashion, baby, auto. Realistic ASINs starting with B0. Return ONLY the JSON array.`;

    try {
      const d = await callAI({
        messages: [{ role: "user", content: prompt }],
        max_tokens: 2000,   // 10 products × ~180 tokens each = ~1800, with headroom
      });
      const txt = extractText(d);
      const parsed = extractJSON(txt);
      return Array.isArray(parsed) ? parsed : (parsed.products || []);
    } catch (err) {
      console.error(`Chunk fetch failed (${range.id} offset ${offset}):`, err);
      throw err;
    }
  }, [callAI]);

  // Top-level fetch: chunks until we reach targetCount, shows progress as it goes.
  const fetchRange = useCallback(async (rangeId, force = false) => {
    if (!force && cache[rangeId]?.length >= targetCount) return;

    setLoading(l => ({ ...l, [rangeId]: true }));
    setErrors(e => ({ ...e, [rangeId]: null }));
    setLoadingProgress(p => ({ ...p, [rangeId]: `0/${targetCount}` }));

    const range = ranges.find(r => r.id === rangeId);

    // Build initial avoid list from OTHER tabs so products don't repeat
    let accumulated = [];
    const otherTabNames = Object.entries(cache)
      .filter(([r]) => r !== rangeId)
      .flatMap(([, prods]) => (prods || []).map(p => p.name))
      .filter(Boolean);

    const chunkSize = 25; // Claude handles ~25 products per call comfortably
    let failedChunks = 0;

    for (let offset = 0; offset < targetCount; offset += chunkSize) {
      const needed = Math.min(chunkSize, targetCount - offset);
      const avoid = [...otherTabNames, ...accumulated.map(p => p.name)];

      try {
        const chunk = await fetchChunk(range, avoid, offset, needed);
        if (!chunk.length) { failedChunks++; break; }
        // Dedupe within this range in case AI repeats
        const existingNames = new Set(accumulated.map(p => p.name?.toLowerCase()));
        const fresh = chunk.filter(p => p.name && !existingNames.has(p.name.toLowerCase()));
        accumulated = [...accumulated, ...fresh];
        // Progressive update: user sees products appearing as chunks arrive
        const withIds = accumulated.map((p, i) => ({
          ...p, id: `${rangeId}-${i}-${p.asin || (p.name?.slice(0,8) || Math.random())}`,
        }));
        setCache(c => ({ ...c, [rangeId]: withIds }));
        setLoadingProgress(p => ({ ...p, [rangeId]: `${accumulated.length}/${targetCount}` }));
      } catch {
        failedChunks++;
        if (failedChunks >= 2) break; // bail after 2 consecutive failures
      }
    }

    if (accumulated.length === 0) {
      // Total failure — fallback to seeded HOT_PRODUCTS
      ...
      setErrors(e => ({ ...e, [rangeId]: "AI refresh failed" }));
    }

    setLastUpdated(u => ({ ...u, [rangeId]: Date.now() }));
    setLoading(l => ({ ...l, [rangeId]: false }));
  }, [cache, fetchChunk, targetCount]);

  // Load active range on mount / switch
  useEffect(() => {
    if (!cache[activeRange] || cache[activeRange].length < targetCount) {
  ...
  const progress     = loadingProgress[activeRange] || "";
    ...
    <div style={{ marginTop:24 }}>
      ...
      <div style={{ marginBottom:14 }}>
        <div style={{ fontFamily:"Syne,sans-serif", fontWeight:900, fontSize:16, color:"#f0f0f0" }}>
          📊 AI-Researched <span style={{ color:accentColor }}>Trending</span>
        </div>
        <div style={{ fontSize:10, color:"#444", marginTop:2 }}>
          Up to {targetCount} products per tab · ranked by sales potential · no overlaps
        </div>
      </div>
...
function InlineAmazonLookup({ tier, platforms, accentColor, onAnalyze }) {
  ...
  const search = async () => {
    ...
    const prompt = isAsin
      ? `You are an Amazon product analyst. ASIN: ${q.toUpperCase()}
Return ONLY a raw JSON object (no markdown, no backticks):
{"name":"full product title","brand":"Brand","asin":"${q.toUpperCase()}","category":"Home","platforms":["Amazon"],"surge":72,"potential":78,"sales30d":35000,"reviews":14200,"rating":4.5,"price":"$29.99","origPrice":null,"onSale":false,"trend":"+84%","tags":["trending"],"description":"Two sentences why it sells.","pros":["Strength 1","Strength 2","Strength 3"],"cons":["Risk 1","Risk 2"]}`
      : `You are an Amazon product analyst. A seller is looking up: "${q}"
Find the single best-matching Amazon bestseller and return ONLY a raw JSON object (no markdown):
{"name":"exact realistic product title","brand":"Brand","asin":"B0XXXXXXXXX","category":"category","platforms":["Amazon"],"surge":74,"potential":81,"sales30d":42000,"reviews":16800,"rating":4.6,"price":"$24.99","origPrice":null,"onSale":false,"trend":"+120%","tags":["tag1","tag2"],"description":"Two sentences on why it sells and who buys it.","pros":["Strength 1","Strength 2","Strength 3"],"cons":["Risk 1","Risk 2"]}
Use a realistic product name, price, sales velocity, and ASIN starting with B0. Return ONLY the JSON.`;
    ...
    try {
      ...
      const txt    = extractText(d);
      const parsed = extractJSON(txt);
      if (!parsed.name) throw new Error("Incomplete product data returned");

      // Merge computed scores into the product so surge/potential/risk all show
      const scores  = computeProductScores(parsed);
      const product = {
        ...parsed,
        id:        Date.now() + Math.random(),
        // Ensure required fields exist
        platforms: parsed.platforms || ["Amazon"],
        tags:      parsed.tags      || [],
        trend:     parsed.trend     || "+N/A",
        // Attach computed potential so ScoreRing shows it correctly
        potential: parsed.potential || scores.packageScore,
        _scores:   scores,
        _fromLookup: true,
      };
      ...
      setError(err.message || "Could not find product. Try a different name or ASIN.");
    }
    setLoading(false);
  };

  const amazonUrl = (p) => p.asin && !/^B0X+$/i.test(p.asin)
    ? `https://www.amazon.com/s?k=${encodeURIComponent(p.name + " " + p.asin)}`
    : `https://www.amazon.com/s?k=${encodeURIComponent(p.name)}`;
            ...
            {tier === "free" && (
              <div style={{ marginBottom:14, padding:"10px 14px",
                ...
                <span>Free: 5 curated + 10 trending per tab.{" "}
                  ...
                  <strong style={{ color:"#fbbf24" }}>Pro ($30/mo)</strong>: 100 trending per tab.
                </span>
