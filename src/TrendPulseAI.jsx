...
Product: ${product.name} | Category: ${product.category} | Price: ${product.price}
Trend: ${product.trend} | 30d Sales: ${product.sales30d} | Reviews: ${product.reviews} (${product.rating}★)
Surge: ${product.surge}/100 | Potential: ${product.potential}/100 | Platforms: ${product.platforms.join(", ")}
...
3. **Platform Strategy**
4. **Profit Potential**
5. **Action Steps** (3 this week)
6. **Risk Watch**`
      : `You are an e-commerce bundling strategist. Sale product:
${product.name} | Sale: ${product.price} | Was: ${product.origPrice}
Surge: ${product.surge}/100 | Potential: ${product.potential}/100

Provide:
1. **Sale Opportunity Score** (1-10)
2. **Why Buy Now**
3. **Best Platform to Resell**
4. **Bundle Recommendation**
5. **Projected Bundle Revenue**
6. **Timing Advice**`;

    // Stream the response — text appears as it's generated for 10x faster perceived speed
    ...
    callAIStream({
      ...
      requireDeep: true,
      onDelta: (chunk) => {
        if (!gotFirstChunk) { setLoading(false); gotFirstChunk = true; }
...
function PackageAnalysisModal({ pkg, onClose }) {
  ...
  useEffect(() => {
    ...
    const itemList = items.map(p => `${p.name} (${p.price}, Surge: ${p.surge})`).join(", ");
    const prompt =
`You are an e-commerce bundle strategist. Analyze this bundle deeply:
Bundle: ${pkg.name}
Theme: ${pkg.theme}
Items: ${itemList}
Target Audience: ${pkg.targetAudience}
Potential Uplift: ${pkg.potentialUplift}
Bundle Savings: ${pkg.bundleSavings}

Provide:
1. **Bundle Market Opportunity**
2. **Ideal Buyer Persona**
3. **Best Platforms for This Bundle**
4. **Pricing Strategy**
5. **Marketing Angle**
6. **Revenue Projection (30 / 90 days)**
7. **Risk Factors**`;
    ...
    callAIStream({
      ...
      .catch(err => { setText("⚠️ " + err.message + "\n\nPlease try again."); setLoading(false); });
  }, [pkg, callAIStream]);
...
function RiskPackageModal({ product, scores, onClose }) {
  ...
  useEffect(() => {
    ...
    const prompt =
`You are a professional e-commerce risk analyst and merchandise bundling strategist.

Product: ${product.name}
Category: ${product.category} | Price: ${product.price}
30-Day Sales Estimate: ${(product.sales30d || 0).toLocaleString()} units
Review Count: ${(product.reviews || 0).toLocaleString()} | Rating: ${product.rating}★
Trend: ${product.trend} | Surge Score: ${product.surge}/100 | Potential: ${product.potential}/100
Computed Risk Score: ${riskScore}/100 (${riskLevel} Risk)
Computed Package Score: ${packageScore}/100 (${packageLevel} Bundle Potential)

Provide a comprehensive actionable analysis:
1. **Risk Breakdown** — supply chain, market saturation, trend sustainability
2. **Competition Intensity** — market crowding and differentiation opportunities
3. **Sales Velocity Analysis** — what the 30d sales figure signals
4. **Bundle Recommendations** — 3 specific complementary products with rationale
5. **Optimal Bundle Price Point** — pricing strategy and margin guidance
6. **30-Day Action Plan** — 5 concrete, prioritized steps
7. **Verdict** — Buy In / Build Bundle / Hold / Pass (with reasoning)`;
        ...
        {activeTab === "hot" && (
          <div>
            <div style={{ display:"flex", justifyContent:"space-between",
              ...
              <div>
                <div style={{ fontFamily:"Syne,sans-serif", fontWeight:900, fontSize:20, color:"#f0f0f0" }}>
                  🔥 Top {tier==="free"?5:tier==="growth"?20:"∞"}{" "}
                  <span style={{ color:ac }}>Hot Items</span>
                </div>
                <div style={{ fontSize:11, color:"#444", marginTop:3 }}>
                  Ranked by yesterday's cross-platform sales velocity
                </div>
              </div>
            </div>
            {tier === "free" && (
            ...
            />
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(min(100%,320px),1fr))", gap:14, marginTop:24 }}>
              {visibleHot.map((p, i) => (
