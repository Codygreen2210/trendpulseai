...
function TrendingProductsSection({ tier, accentColor, platforms, onAnalyze }) {
  ...
  const targetCount = tier === "free" ? 10 : 50; // Growth + Pro both get 50 per tab
  const perPage = 10;
...
Mix categories widely: home, kitchen, beauty, tech, fitness, pets, office, outdoor, fashion, baby, auto. Realistic ASINs starting with B0. Return ONLY the JSON array.`;
  ...
  const fetchRange = useCallback(async (rangeId, force = false) => {
    ...
    // PARALLEL LOADING: fire all chunks at once instead of waiting one at a time.
    // 2 parallel calls × 25 products each = ~10-15s total (vs 30s+ sequential).
    // Chunks can't know about each other while in flight, so we dedupe client-side.
    const chunkSize = 25;
    const chunksNeeded = Math.ceil(targetCount / chunkSize);
    ...
    for (let i = 0; i < chunksNeeded; i++) {
      ...
      chunkPromises.push(
        fetchChunk(range, otherTabNames, offset, needed)
          .then(chunk => {
            // Stream each chunk into state as it arrives — feels progressive
            setCache(c => {
              const existing = c[rangeId] || [];
              const existingNames = new Set(existing.map(p => p.name?.toLowerCase()));
              const fresh = chunk.filter(p => p.name && !existingNames.has(p.name.toLowerCase()));
              const merged = [...existing, ...fresh].map((p, idx) => ({
                ...p,
                id: p.id || `${rangeId}-${idx}-${p.asin || (p.name?.slice(0,8) || Math.random())}`,
              }));
              return { ...c, [rangeId]: merged };
            });
            setLoadingProgress(p => {
              const prev = parseInt((p[rangeId] || "0/0").split("/")[0], 10) || 0;
              return { ...p, [rangeId]: `${prev + chunk.length}/${targetCount}` };
            });
            return chunk;
          })
          .catch(err => {
      ...
      <div style={{ marginBottom: 12, fontSize: 11, color: "#444", fontStyle: "italic",
        ...
        {isLoading && (
          <span style={{ color: accentColor, fontSize: 10, fontStyle: "normal",
            ...
            {progress ? `LOADING ${progress}… (~15s)` : "RESEARCHING… (~15s)"}
          </span>
        )}
      </div>
